...
import { readableStreamToString, CrowdSecError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new CrowdSecError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new CrowdSecError(e, e.response?.data)
                }
                if (innerError instanceof CrowdSecError) {
                    // Got "CrowdSecError" from the above try block
                    throw innerError;
                }
